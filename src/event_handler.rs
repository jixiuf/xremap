...
use crate::config::modmap_action::{ModmapAction, MultiPurposeKey, PressReleaseKey, Keys};
    ...
    fn release(&self) -> Vec<(Key, i32)> {
        if let Some(alone_timeout_at) = &self.alone_timeout_at {
            if Instant::now() < *alone_timeout_at {
                ...
                let release_keys :Vec<(Key,i32)> =release_keys.into_iter()
                    .map(|key| (key, RELEASE)).collect();
                
                let mut keys =self.alone.clone().into_vec();
                keys.sort_by(modifiers_first);
                let mut keys :Vec<(Key,i32)> =keys.into_iter()
                    .map(|key| (key, PRESS)).collect();
